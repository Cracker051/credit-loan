...
from backend.base.const import CreditPlanType, CreditRequestStatusType, TimePeriodType, TransactionType
...
class Transaction(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.__class__.__name__}: {self.type}  - {self.amount}"
...
class CreditRequest(models.Model):
    ...
    repayment_period_unit = models.CharField(choices=TimePeriodType.choices(), max_length=10, default="year")
    ...
    status = models.CharField(
        choices=CreditRequestStatusType.choices(), max_length=10, default=CreditRequestStatusType.PENDING
    )
